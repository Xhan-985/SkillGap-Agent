"""抽取 Prompt（版本化管理——EVALUATION_PLAN §6）。

v2（2026-09-02）：evidence 约束强化——不得跨越列表符号/换行（v1 在
项目符排版的真实 JD 上出现"熟悉或实际使用过: Playwright"式跨行证据，
定位校验失败）。E1 v2 数据集捕获该缺陷。
...
2. evidence_text 必须取自同一行内的一段连续原文，不得跨越列表符号（•、-、*、→）或换行。若技能出现在项目符列表中，取该项内的短语（如技能名本身），不要把列表引导语和列表项拼在一起。
3. 只抽技术技能（语言/框架/工具/平台/方法论）。不抽：学历、年限、软素质（沟通能力等）、公司福利、业务领域名词。
4. soft_requirements 数组：type 取 experience（年限）/education（学历）/language（语言要求），value 为原文值，evidence_text 同样必须是原文连续片段且不跨行。
5. 召回优先：凡原文明确提及的技术技能都抽取，不确定的宁可不抽；但不要发明原文没有的技能。
6. 输出仅一个 JSON 对象：{"skills": [...], "soft_requirements": [...]}，无任何其他文本。
