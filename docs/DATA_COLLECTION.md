...
- **推荐用交互式收集器录入**：`skillgap collect --out data/batch_1.csv`
  - **粘贴优先**：先粘贴 JD 全文（单独一行 END 结束），然后自动识别并逐项回车确认——岗位名称（首行/招聘语句提取）、公司（"XX公司"）、城市（22 城名单）、国家（中文默认中国）、薪资范围（15k-25k 等）、岗位类别（8 枚举规则归类）、技能建议（词表 alias 扫描 + 证据原文 + 强度词）；PII 自动替换、质检提醒
