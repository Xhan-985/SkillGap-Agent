...
- **推荐用交互式收集器录入**：`skillgap collect --out data/batch_1.csv`
  - **终端粘贴丢字时用 `--file`**：`skillgap collect --file jd.txt`——JD 存成 UTF-8 文本文件直接读，彻底绕开控制台多行粘贴截断/乱码。**连续模式**：每条保存后不退出——更新 jd.txt 内容 → 回车读下一条，`q` 退出；内容未变时自动跳过（防重复录入），逐条追加到同一 CSV
  - **粘贴优先**：先粘贴 JD 全文（单独一行 END 结束），粘贴结束会回显"已接收 N 行 / M 字符"（明显少于原文=粘贴被截断，改用 --file），然后自动识别并逐项回车确认——岗位名称（首行/招聘语句提取）、公司（"XX公司"）、城市（22 城名单）、国家（中文默认中国）、薪资范围（15k-25k 等；无单位大数须带"元/薪"上下文，日期 2026-08-25 不会误判）、岗位类别（标题优先两阶段归类）、技能建议（词表 alias 扫描 + 证据原文 + 强度词）；PII 自动替换、质检提醒
  - 唯一必须手填的是**来源 URL**（浏览器地址栏复制，不在 JD 正文里）
  - 技能逐条确认（[m]必须 [n]加分 [s]跳过；标注"推断"=由措辞推断而非技术名命中）——**建议不代替标注**，importance 由人定
  - **通用技术词**（如只写"向量数据库/图数据库"不写具体产品）：词表不再挂具体产品（v1.1 已移除），收集时不建议、人工标注也不归一——导入后进 `new_skill_candidate` 人工裁决；JD 里出现 RAG/Embedding 等未收录概念同理
